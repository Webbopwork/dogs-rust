...
fn main() {
    ...
    if args.len() > 1 {
        ...
        match process(&client, &args[1]) {
            Ok(_) => println!("\nProcess over!"),
            Err(e) => println!("Got error: {}\nEnding process...", e)
        }
    } else {
        println!("No argument for address found, ending process...");
    }
}

fn process(dog: &dogs::ConnectedDog, address_string: &String) -> io::Result<()> {
